...
def compute_cotangent(a, b, c, A):
    ''' Compute the cotangent of an angle in a triangle by using the triangle edge lengths and area only. The input parameters are defined in the handout figure. The purpose of this function is to check that your formula is correct. You should not directly use this in the `compute_cotangent_weights_matrix` function.
